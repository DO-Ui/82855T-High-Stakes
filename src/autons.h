...
inline void skills(){
	...
	conveyor.move(127);
	delay(150);
	chassis.turnToPoint(-24.075, -24.75, 600, {.earlyExitRange=15}); // turn to first ring
	chassis.moveToPoint(-24.075, -24.75, 600); //move to first ring
	chassis.turnToPoint(26, -46, 700, {.earlyExitRange=10}); // score far ring
	chassis.moveToPoint(0, -40, 500, {.minSpeed=127, .earlyExitRange=7}); // motion chain to avoid ladder
	chassis.moveToPoint(24, -48.7, 1100, {.minSpeed=60, .earlyExitRange=3}); //move to far ring
	delay(400);
	set_LBPosition(1);
	...
	chassis.moveToPoint(-62, -50, 1200, {.maxSpeed = 55}); //grab 2 rings in a line
	chassis.turnToPoint(-47.84, -61, 700, {.earlyExitRange=5});
	chassis.moveToPoint(-47.84, -61, 1000); //grab last corner ring
	// chassis.turnToPoint(-59, -62, 700, {.forwards = false});
	chassis.turnToHeading(45, 700, {.earlyExitRange=5});
	chassis.moveToPoint(-62, -65, 500, {.forwards=false, .earlyExitRange=3}); //drive to corner
	chassis.waitUntilDone();
	mogoclamp.retract();
	conveyor.move(-127); //reverse conveyor to unjam it
	intake.move(0);
	delay(250);
	chassis.moveToPoint(0, -50, 1000, {.minSpeed=110, .earlyExitRange=5}); // drive to intermediate point
	intake.move(-127); // NOTE: if this causes problems, slow intake speed later
	chassis.moveToPoint(35, -50, 2800, {.minSpeed = 30, .earlyExitRange = 5}); //move to intermediate point
	conveyor.move(0);
	chassis.moveToPoint(52, -50, 800, {.minSpeed = 30}); //move to lower right corner first ring (for alliance stake)
	chassis.waitUntilDone();
	conveyor.move(127);
	...
	chassis.turnToHeading(227, 800);
	intake.move(0);
	chassis.moveToPoint(59+5, -32-7+5, 800, {.forwards = false, .maxSpeed=80}); //move to bottom blue mogo
	chassis.swingToHeading(180, lemlib::DriveSide::RIGHT, 800, {.maxSpeed = 100, .minSpeed = 30, .earlyExitRange = 5}); //swing to mogo
	...
	delay(300);
	temp = chassis.getPose();
	chassis.setPose(62.7, 0, temp.theta, false);
	intake.move(-127);
	conveyor.move(127);
	delay(1200);
	conveyor.move(0);
	intake.move(0);
	chassis.moveToPoint(48.5, 0, 600, {.maxSpeed = 80}); //move away from alliance stake
	chassis.turnToHeading(180, 700, {.earlyExitRange=10});
	chassis.moveToPoint(46.5, 9, 700, {.forwards = false, .earlyExitRange=2}); //go to empty mogo
	approachAndClampMogo();
	delay(100);
	chassis.turnToPoint(37, -7, 800, {.earlyExitRange=10});
	chassis.moveToPoint(37, -7, 1500, {.minSpeed=50, .earlyExitRange=5}); //move to intermediate point
	conveyor.move(127);
	intake.move(-127);
	chassis.moveToPoint(21, -28, 900); //move to ring closest to the middle in the lower right corner
	delay(400);
	chassis.turnToPoint(-21.492, 20, 900, {.earlyExitRange=5});
	chassis.moveToPoint(-21.492, 20, 1700, {.maxSpeed = 90}); //move to ring after the middle ring
	delay(300);
	conveyor.move(0);
	chassis.waitUntilDone();
	conveyor.move(127);
	...
	chassis.tank(-50, -50);
	delay(450);
	chassis.moveToPoint(-2, 53, 600, {.forwards = false});
	set_LBPosition(0);
	chassis.turnToPoint(23.713, 53, 600);
	chassis.moveToPoint(23.713, 53, 600); //move to first ring in upper right corner
	delay(400);
...
inline void redMogoRush(){
	...
	chassis.moveToPoint(-22, -60, 1000, {.minSpeed = 120, .earlyExitRange=10}); // intermediate point
	chassis.moveToPoint(-11.3, -58, 600, {.minSpeed=100, .earlyExitRange=1}); //rush mogo
	// chassis.turnToHeading(69, 300, {.earlyExitRange=5});
	chassis.waitUntil(12.4); // TUNE THIS VALUE
	...
	chassis.moveToPoint(-22, -58, 800, {.forwards=false});
	chassis.swingToHeading(200, lemlib::DriveSide::RIGHT, 800, {.direction=lemlib::AngularDirection::CCW_COUNTERCLOCKWISE});
	// chassis.turnToHeading(230, 500); // turn around to face mogo clamp to mogo
	chassis.waitUntilDone();

	chassis.moveToPose(-11.5, -48, 190, 800, {.forwards=false, .lead=0.2}, false); // move to clamp mogo
	chassis.waitUntilDone();
	approachAndClampMogo();
	conveyor.move(127);
	chassis.moveToPoint(-22, -60, 800); // move outta the way of ring stack
	chassis.turnToPoint(-38, -60, 800);
	chassis.moveToPoint(-38, -60, 800); // move back to drop off mogo
	chassis.swingToHeading(90, lemlib::DriveSide::RIGHT, 1200, {.direction=lemlib::AngularDirection::CW_CLOCKWISE}); // swing to face ring stack
	mogoclamp.toggle(); // drop mogo
	intake.move(-127);
	chassis.turnToPoint(-40, -47, 800); // turn to the blue ring
	sorter_active=false;
	chassis.moveToPoint(-40, -47, 800); // move to throw away blue single ring
	chassis.waitUntilDone();
	chassis.turnToPoint(-23, -46, 800);
	sorter_active=true;
	chassis.moveToPoint(-23, -46, 900); // move to ring stack
	stopNextRing=true;
	chassis.turnToPoint(-23.5, -20, 900, {.forwards=false}); // move to second mogo
	chassis.moveToPoint(-23.5, -20, 900, {.forwards=false}); // move to second mogo
	chassis.waitUntilDone();
	approachAndClampMogo();
	stopNextRing=false;
	conveyor.move(127);
